import { GoalDataAggregated, TasksDataAggregated } from '@/app/page';
...
export default function Goals({ tasksState, goalsState, setGoalsState }: 
    {
    ...
    return (
        ...
        { Object.entries(goalsState).map(([goalPosition, goal]) => {
          ...
          return (
            <div key={`${goalPosition}`} className="flex relative w-80 h-full p-[5px] bg-[#1C2127] border-[1px] border-[#666666] rounded-[5px]">
                            ...
                            return (
                                ...
                                { Object.entries(milestone.steps).map(([stepPosition, step]) => {
                                    ...
                                    return (
                                        <h4 
                                        key={`${stepPosition}`} 
                                        className={`flex flex-col flex-1 w-full min-w-0 hover:border-1 border-blue-500 leading-none`}
                                        style={{ 
                                            fontFamily: "'Inter'", 
